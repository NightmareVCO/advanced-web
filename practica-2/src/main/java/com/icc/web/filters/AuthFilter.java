...
public class AuthFilter extends OncePerRequestFilter {
  ...
  private static final List<Pattern> SECURED_PATH_PATTERNS = List.of(
      Pattern.compile("/api/v1/endpoint/"),
      Pattern.compile("/api/v1/projects/"));

  @Override
  protected boolean shouldNotFilter(@NonNull HttpServletRequest request) {
    String requestURI = request.getRequestURI();
    return SECURED_PATH_PATTERNS.stream().noneMatch(pattern -> pattern.matcher(requestURI).matches());
  }
  ...
  protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response,
    ...
    String projectId = pathMatcher
        .extractUriTemplateVariables("/api/v1/endpoint/projects/{projectId}/api/**", request.getRequestURI())
        .get("projectId");
