...
public class ProjectController {
    ...
    public ResponseEntity<List<ProjectResponseDTO>> getAllProjects(
        ...
        List<Project> projects =
                projectService.getAllProjectsByUserId(user.get().getId(), isAdminRole);
        List<ProjectResponseDTO> projectsResponseDTO =
                ProjectMapper.INSTANCE.projectsToResponseDtos(projects);
    ...
    public ResponseEntity<ProjectResponseDTO> getProjectById(
        ...
        boolean isUserPartOfTeam =
                project.get().getTeam().stream().anyMatch(u -> u.getUsername().equals(username));
        boolean isAdminRole =
                user.get().getRoles().stream().anyMatch(role -> role.getName().equals(ERole.ADMIN));
        if ((!username.equals(ownerUsername) && !isProjectPublic && !isUserPartOfTeam)
                && !isAdminRole) {
            throw new ForbiddenException("You are not allowed to view this project");
        }

        ProjectResponseDTO projectResponseDTO =
                ProjectMapper.INSTANCE.projectToResponseDto(project.get());
    ...
    public ResponseEntity<ProjectDTO> addUserToTeam(
    ...
    public ResponseEntity<ProjectDTO> removeUserFromTeam(
            @PathVariable String username,
            @PathVariable Long projectId,
            @RequestHeader("Authorization") String token) {
        Optional<Project> project = projectService.getProjectById(projectId);
    ...
    public ResponseEntity<ProjectResponseDTO> createProject(
        ...
        ProjectResponseDTO createdProjectDTO =
                ProjectMapper.INSTANCE.projectToResponseDto(savedProject.get());
    ...
    public ResponseEntity<ProjectResponseDTO> updateProject(
        ...
        existingProject.get().setDescription(projectDTO.getDescription());

        Optional<User> ownerFromDb =
                userService.getUserById(existingProject.get().getOwner().getId());
        ...
        ProjectResponseDTO updatedProjectDTO =
                ProjectMapper.INSTANCE.projectToResponseDto(savedProject.get());
