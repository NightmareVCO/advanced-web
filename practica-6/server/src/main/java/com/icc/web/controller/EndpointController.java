...
public class EndpointController {
    ...
    public ResponseEntity<List<EndpointResponseDTO>> getAllEndpoints() {
        ...
        List<EndpointResponseDTO> endpointsResponseDTOs =
                EndpointMapper.INSTANCE.endpointsToResponseDtos(endpoints);
    ...
    public ResponseEntity<EndpointResponseDTO> getEndpointById(@PathVariable Long id) {
        ...
        EndpointResponseDTO endpointDTO =
                EndpointMapper.INSTANCE.endpointToResponseDto(endpointOptional.get());
        if (endpointDTO == null) {
            throw new InternalServerError("Internal Server Error");
        }
    ...
    public ResponseEntity<EndpointResponseDTO> createEndpoint(
        ...
        if (endpointService
                .getEndpointByPathAndMethod(endpointDTO.getPath(), endpointDTO.getMethod())
                .isPresent()) {
        ...
        if (endpoint.isSecurity()) {
            jwt =
                    jwtService.createTokenForEndpoint(
                            endpoint.getProject().getId().toString(),
                            endpoint.getId().toString(),
                            endpoint.getExpirationDate());
            if (jwt != null) {
        ...
        EndpointResponseDTO createdEndpoint =
                EndpointMapper.INSTANCE.endpointToResponseDto(savedEndpoint.get());
    ...
    public ResponseEntity<EndpointResponseDTO> updateEndpoint(
            @PathVariable Long id, @RequestBody EndpointDTO endpointDTO) {
        AuthResponseDTO jwt = null;
        ...
        if (newEndpoint.isSecurity()) {
            jwt =
                    jwtService.createTokenForEndpoint(
                            existingEndpoint.getProject().getId().toString(),
                            existingEndpoint.getId().toString(),
                            newEndpoint.getExpirationDate());
            if (jwt != null) {
        ...
        EndpointResponseDTO updatedEndpointDTO =
                EndpointMapper.INSTANCE.endpointToResponseDto(updatedEndpoint.get());
